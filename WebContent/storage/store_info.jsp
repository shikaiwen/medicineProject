...
$(function(){
	...
	//	view:detailview,
		...
		striped: true,
		collapsible:true,
		url:'<%=base%>store/StoreAction!selectInstoreInfo.action',
		sortName: 'code',
		sortOrder: 'desc',
		remoteSort: false,
		idField:'medicineNo',
...
function viewDetail(storeId){
	var data = new Object();
	data['storeCard.storeId'] = storeId;
//	alert(JSON.stringify(data));
	$.ajax({
		...
		//	alert(datas);
			var medicines = data['resultList'][0];
			var content = data['resultList'][1];
			//alert(JSON.stringify(medicines));
			//alert(JSON.stringify(content));
			initDetail(medicines,content);
		}
	})
}

function initDetail(medicines,content){
	var medicines = medicines['medicines'];
	var content = content['content'];
	
	var targetTable = $("#showDetailTable");
	var table = $("#template2").clone();
	
	for(var v=0;v<medicines.length;v++){
		var header = table.find("#templateHeader").clone();
		header.find("td").eq(1).html(medicines[v]['medicineNo']);
		header.find("td").eq(3).html(medicines[v]['medicineName']);
		header.appendTo(targetTable);
		
		var arr = content[medicines[v]['medicineNo']];
		for(var i=0;i<arr.length;i++){
			if(i%6==0){
				var tr = $("<tr></tr>");
				tr.appendTo(targetTable);
			}
			var td = $("<td></td>").html(arr[i]); td.appendTo(tr);
		}
	}
	$("#detailDlg").dialog({
		width: 1000,  
...
<body class="body1">
	...
	<!-- 显示详细信息对话框 -->
	<div id="detailDlg" class="easyui-dialog" style="margin-right:auto;margin-left:auto;width:300px" closed="true" buttons="#detailShow">
	<div id="detailContentDiv">
		<table id="showDetailTable" class="table2">
		</table>
	</div>
		 <div id="detailShow">  
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#detailDlg').dialog('close')">关闭</a>  
    </div>  

	
	<table id="template2" style="display:none">
		<tr id="templateHeader">
			<td>药品编号:</td><td colspan="2"></td>
			<td>药品名称:</td><td colspan="2"></td>
		</tr>
	</table>
	 <div id="dlg-buttons">  
